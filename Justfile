...
fork:
    ...
    printf 'VITE_ANVIL_RPC_URL=%s\nVITE_RENEWAL_POOL_FACTORY_ADDRESS=%s\nVITE_SEED_POOL_LABELS=%s:ensfairy appraised;%s:ensfairy all\n' "$anvil_rpc_url" "$factory" "$appraised_pool" "$all_pool" > app/.env.local
    printf 'Anvil fork ready at %s\nRenewalPoolFactory: %s\nAdapters: swap=%s yield=%s stream=%s\nFairy pools: appraised=%s all=%s\n' "$anvil_rpc_url" "$factory" "$swap_adapter" "$yield_adapter" "$stream_adapter" "$appraised_pool" "$all_pool"
    wait "$anvil_pid"
