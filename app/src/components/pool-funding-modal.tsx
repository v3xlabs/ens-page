...
export const PoolFundingModal = (properties: PoolFundingModalProperties) => (
  <TransactionModal
    ...
    title={properties.action === "deposit" ? "Fund pool" : "Withdraw from pool"}
  >
    <div class="mt-4 space-y-4">
      <Show when={properties.action === "deposit"}>
        <div class="rounded-button border border-border bg-background-secondary p-3">
          ...
          <p class="mt-1 text-sm text-text-secondary">Available now. Token funding will appear here after a pool adapter is enabled.</p>
        </div>
      </Show>
      <label class="block text-sm font-bold text-text-secondary" for="pool-funding-amount">Amount in ETH</label>
      <input
      ...
      <Show when={properties.action === "withdraw"}>
        <p class="text-sm text-text-secondary">Funds return to your connected wallet.</p>
      </Show>
    </div>
  </TransactionModal>
);
