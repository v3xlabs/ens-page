...
// Faithful port of the ENS manager app's fallback avatar so identical names
// produce identical gradients (ens-app-v3 src/utils/gradient.ts, itself from
// https://github.com/ourzora/zorb). The gradient seeds from namehash(name) —
// the full name, not just a label.
  ...
  switch (option) {
    case 0: {
      return (hue: number, pct: number) => {
        ...
        return clampHue(linear(1 - pct) * hue + linear(pct) * endHue);
      };
    }
    case 1: {
      return (hue: number, pct: number) => {
    ...
    case 2: {
      return (hue: number, pct: number) => {
        ...
        return clampHue(linear(1 - lerpPercent) * hue + lerpPercent * endHue);
      };
    }
    default: {
      return (hue: number, pct: number) => {
        const endHue = hue + multiplier * 60 * bscale(optionNum, 1) + 30;
        const lerpPercent = cubicInOut(pct);

        return clampHue((1 - lerpPercent) * hue + lerpPercent * endHue);
      };
    }
  }
};
...
const lerpLightnessFn = (optionNum: number) => {
  if (optionNum === 0) {
    return (start: number, end: number, pct: number) => {
      ...
      return (1 - lerpPercent) * start + lerpPercent * end;
    };
  }
...
const gradientForBytes = (hash: Hex): string[] => {
  const bytes = hexToBytes(hash).reverse();
  ...
  return inputs.map(input => `hsl(${Math.round(input.h)}, ${Math.round(input.s)}%, ${Math.round(input.l)}%)`);
};

// Kept byte-identical to the manager app's SVG (including its quirks) so the
// same name renders the same data URI there and here.
const zorbImageSVG = (name: string) => {
  const gradientInfo = gradientForBytes(namehash(name));
...
export const zorbImageDataURI = (name: string): string => `data:image/svg+xml;base64,${btoa(zorbImageSVG(name))}`;
